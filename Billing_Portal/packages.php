...
try {
    $stmt = $pdo->query("
        ...
        FROM packages p
        LEFT JOIN f_packages fp
    ON fp.fpkgid = (
        SELECT MIN(f1.fpkgid)
        FROM f_packages f1
        WHERE f1.pkgid = p.id
          AND (
              f1.price IS NOT NULL
              OR f1.customprice IS NOT NULL
              OR f1.cost IS NOT NULL
          )
    )
        LEFT JOIN (
...
$used_packages = 0; foreach ($packages as $pkg_for_count) { if ((int)$pkg_for_count["user_count"] > 0) { $used_packages++; } }
...
@keyframes heroIn{from{opacity:0;transform:translateY(-12px) scale(.98)}to{opacity:1;transform:translateY(0) scale(1)}}
@keyframes iconFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}
.hero{animation:heroIn .6s ease-out both}
.stats .stat{animation:fadeUp .55s ease-out both}
.stats>div:nth-child(1) .stat{animation-delay:.08s}.stats>div:nth-child(2) .stat{animation-delay:.16s}.stats>div:nth-child(3) .stat{animation-delay:.24s}.stats>div:nth-child(4) .stat{animation-delay:.32s}
.toolbar{animation:fadeUp .55s ease-out .35s both}
.package-card{animation:fadeUp .5s ease-out both}
.package-card:nth-child(1){animation-delay:.08s}.package-card:nth-child(2){animation-delay:.14s}.package-card:nth-child(3){animation-delay:.20s}.package-card:nth-child(4){animation-delay:.26s}.package-card:nth-child(5){animation-delay:.32s}.package-card:nth-child(6){animation-delay:.38s}
.pkg-icon{animation:iconFloat 3s ease-in-out infinite}
.pkg{transition:transform .25s ease,box-shadow .25s ease}
.pkg:hover{transform:translateY(-6px) scale(1.01);box-shadow:0 16px 32px rgba(24,39,75,.11)}
.btn-users,.btn-view{transition:transform .18s ease,box-shadow .18s ease}
.btn-users:hover,.btn-view:hover{transform:translateY(-1px)}
.modal-content{animation:fadeUp .3s ease-out}
