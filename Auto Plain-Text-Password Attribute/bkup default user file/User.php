	...
	public function changepassword()
	{
		...
		if ($this->main->getUserinfoByID($userID)) {
			...
			if ($this->form_validation->run()) {
				...
				$data['portalpass'] = md5($this->input->post('password', true));
				$this->db->where('id', $userID);
				$true = $this->db->update('usersinfo', $data);
				$username = $userData->username;
				$data2['value'] = $this->input->post('password', true);
				...
				else if (($userData->status == 2) && ($userData->connectionstatus == 1)) {
					$data3['username'] = $userData->username;
					$data3['attribute'] = 'Cleartext-Password';
					$data3['op'] = ':=';
					$data3['value'] = $this->input->post('password', true);
					$this->main->singleInsert('radcheck', $data3);
				}
